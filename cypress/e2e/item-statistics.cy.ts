import {
  REGEX_MATCH_NON_EMPTY_TEXT,
  TEST_ENTITY_PUBLICATION,
} from 'cypress/support/e2e';
import { testA11y } from 'cypress/support/utils';

describe('Item Statistics Page', () => {
  const ITEMSTATISTICSPAGE = '/statistics/items/'.concat(
    TEST_ENTITY_PUBLICATION,
  );
  ...
  it('should contain a "Total visits per month" section', () => {
    ...
    cy.get(
      '.'.concat(TEST_ENTITY_PUBLICATION).concat('_TotalVisitsPerMonth'),
    ).should('exist');
  });
  ...
  it('should pass accessibility tests', () => {
    ...
    cy.get(
      'table[data-test="TotalVisits"] th[data-test="statistics-label"]',
