...
const page = {
    openLoginMenu() {
        ...
        cy.get('ds-navbar [data-test="login-menu"]').click();
    },
    openUserMenu() {
        // Once logged in, click the User menu in header
        cy.get('ds-navbar [data-test="user-menu"]').click();
    },
    submitLoginAndPasswordByPressingButton(email, password) {
        // Enter email
        cy.get('ds-navbar [data-test="email"]').type(email);
        // Enter password
        cy.get('ds-navbar [data-test="password"]').type(password);
        // Click login button
        cy.get('ds-navbar [data-test="login-button"]').click();
    },
    submitLoginAndPasswordByPressingEnter(email, password) {
        // In opened Login modal, fill out email & password, then click Enter
        cy.get('ds-navbar [data-test="email"]').type(email);
        cy.get('ds-navbar [data-test="password"]').type(password);
        cy.get('ds-navbar [data-test="password"]').type('{enter}');
    },
    submitLogoutByPressingButton() {
        // This is the POST command that will actually log us out
        cy.intercept('POST', '/server/api/authn/logout').as('logout');
        // Click logout button
        cy.get('ds-navbar [data-test="logout-button"]').click();
...
describe('Login Modal', () => {
    ...
    it('should allow new user registration', () => {
        ...
        cy.get('ds-navbar [data-test="register"]').should('be.visible');

        // Click registration link & you should go to registration page
        cy.get('ds-navbar [data-test="register"]').click();
    ...
    it('should allow forgot password', () => {
        ...
        cy.get('ds-navbar [data-test="forgot"]').should('be.visible');

        // Click link & you should go to Forgot Password page
        cy.get('ds-navbar [data-test="forgot"]').click();
