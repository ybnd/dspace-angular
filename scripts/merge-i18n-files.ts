...
function parseCliInput() {
  program
    .option(
      ...
      'output dir when running script on all language files',
      projectRoot(LANGUAGE_FILES_LOCATION)
    )
    .option(
      '-s, --source-dir <source-dir>',
      'source dir of transalations to be merged'
    )
  ...
  if (program.outputDir && program.sourceDir) {
    if (
      !fs.existsSync(program.outputDir) &&
      !fs.lstatSync(program.outputDir).isDirectory()
    ) {
    ...
    if (
      !fs.existsSync(program.sourceDir) &&
      !fs.lstatSync(program.sourceDir).isDirectory()
    ) {
    ...
    fs.readdirSync(projectRoot(program.sourceDir)).forEach((file) => {
      if (fs.existsSync(program.outputDir + '/' + file)) {
        ...
        mergeFileWithSource(
          program.sourceDir + '/' + file,
          program.outputDir + '/' + file
        );
      }
    });
  } else {
...
function mergeFileWithSource(pathToSourceFile, pathToOutputFile) {
  ...
  fs.writeFileSync(
    pathToOutputFile,
    JSON5.stringify(parsedOutput, { space: '\n  ', quote: '"' }),
    { encoding: 'utf8' }
  );
