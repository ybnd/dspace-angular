...
function checkJSONResponse(responseData: any): any {
  ...
  try {
    ...
    console.log(
      `\t"dspaceServer" property matches UI's "rest" config? ${parsedData.dspaceServer === appConfig.rest.baseUrl}`,
    );
    ...
    console.log(
      `\tDoes "/api" endpoint have HAL links ("_links" section)? ${linksFound.includes('authn') && linksFound.includes('sites')}`,
    );
  } catch (err) {
    console.error(
