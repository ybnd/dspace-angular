  ...
  for (const property in config) {
    ...
    if (isNotEmpty(innerConfig)) {
      if (typeof innerConfig === 'object') {
        ...
        if (isNotEmpty(value)) {
          console.log(
            `Applying environment variable ${DSPACE(variable)} with value ${value}`,
          );
          switch (typeof innerConfig) {
            ...
            default:
              console.warn(
                `Unsupported environment variable type ${typeof innerConfig} ${DSPACE(variable)}`,
              );
          }
        }
      }
...
export const buildAppConfig = (destConfigPath?: string): AppConfig => {
  ...
  if (isNotEmpty(destConfigPath)) {
    ...
    console.log(
      `Angular ${bold('config.json')} file generated correctly at ${bold(destConfigPath)} \n`,
    );
  }
