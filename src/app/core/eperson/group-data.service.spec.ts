...
describe('GroupDataService', () => {
  ...
  function init() {
    ...
    rdbService = getMockRemoteDataBuildServiceHrefMap(undefined, {
      'https://rest.api/server/api/eperson/groups': groups$,
    });
  ...
  describe('searchNonMemberGroups', () => {
    ...
    it('search with empty query and a group ID', () => {
      ...
      const options = Object.assign(new FindListOptions(), {
        searchParams: [
          Object.assign(new RequestParam('query', '')),
    ...
    it('search with query and a group ID', () => {
      ...
      const options = Object.assign(new FindListOptions(), {
        searchParams: [
          Object.assign(new RequestParam('query', 'test')),
          Object.assign(new RequestParam('group', GroupMock.id)),
        ],
      });
      expect(service.searchBy).toHaveBeenCalledWith(
  ...
  describe('addSubGroupToGroup', () => {
    beforeEach(() => {
      objectCache.getByHref.and.returnValue(
        observableOf({
    ...
    it('should invalidate the previous requests of the parent group', () => {
      ...
      expect(rdbService.buildFromRequestUUIDAndAwait.calls.argsFor(0)[0]).toBe(
        requestService.generateRequestId(),
      );
      const callback =
        rdbService.buildFromRequestUUIDAndAwait.calls.argsFor(0)[1];
      callback();

      expect(objectCache.getByHref).toHaveBeenCalledWith(
        GroupMock._links.self.href,
      );
