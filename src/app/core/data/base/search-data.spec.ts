 ...
 */
export function testSearchDataImplementation(
  serviceFactory: () => SearchData<any>,
) {
  let service;
  ...
  describe('SearchData implementation', () => {
    ...
    it('should handle calls to searchBy', () => {
      const out: any = service.searchBy(
        'searchMethod',
        OPTIONS,
        false,
        true,
...
describe('SearchDataImpl', () => {
  ...
  function initTestService(): SearchDataImpl<any> {
    ...
    constructSearchEndpointSpy = jasmine
      .createSpy('constructSearchEndpointSpy')
      .and.callFake(constructSearchEndpointDefault);
  ...
  describe('getSearchEndpoint', () => {
    ...
    it('should use constructSearchEndpoint to construct the search endpoint', (done) => {
      (service as any).getSearchEndpoint('testMethod').subscribe(() => {
        expect(constructSearchEndpointSpy).toHaveBeenCalledWith(
          'https://rest.api/core',
          'testMethod',
        );
        done();
  ...
  describe('getSearchByHref', () => {
    ...
    it('should return the search endpoint with additional query parameters', (done) => {
      service
        .getSearchByHref('testMethod', options, ...linksToFollow)
        .subscribe((href) => {
          expect((service as any).getSearchEndpoint).toHaveBeenCalledWith(
            'testMethod',
          );
          ...
          expect(href).toBe(
            'https://rest.api/core/search/testMethod?page=2&size=5&embed=a&embed=b',
          );
