...
export class ObjectUpdatesEffects {
   ...
   */
  mapLastNotificationActions$ = createEffect(
    () =>
      this.actions$.pipe(
        ofType(...Object.values(NotificationsActionTypes)),
        map((action: RemoveNotificationAction) => {
          const id: string =
            action.payload.id || action.payload || this.allIdentifier;
          if (hasNoValue(this.notificationActionMap$[id])) {
            this.notificationActionMap$[id] =
              new Subject<NotificationsActions>();
          }
   ...
   */
  removeAfterDiscardOrReinstateOnUndo$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((action: DiscardObjectUpdatesAction) => {
        ...
        let removeAction: Action = new RemoveObjectUpdatesAction(
          action.payload.url
        );
        ...
        return observableRace(
          ...
          this.notificationActionMap$[notification.id].pipe(
            filter(
              (notificationsAction: NotificationsActions) =>
                notificationsAction.type ===
          ...
          this.notificationActionMap$[this.allIdentifier].pipe(
            filter(
              (notificationsAction: NotificationsActions) =>
                notificationsAction.type ===
                NotificationsActionTypes.REMOVE_ALL_NOTIFICATIONS
            ),
            map(() => {
              return removeAction;
            })
