      ...
      switchMap((action: AuthenticateAction) => {
        return this.authService
          ...
          .pipe(
            ...
            catchError((error) =>
              observableOf(new AuthenticationErrorAction(error))
            )
          );
      })
    )
    ...
    this.actions$.pipe(
      ...
      switchMap((action: AuthenticatedAction) => {
        return this.authService.authenticatedUser(action.payload).pipe(
          ...
          catchError((error) =>
            observableOf(new AuthenticatedErrorAction(error))
          )
        );
      })
    )
    ...
    this.actions$.pipe(
      ...
      switchMap((action: RetrieveAuthenticatedEpersonAction) => {
        ...
        return user$.pipe(
          ...
          catchError((error) =>
            observableOf(new RetrieveAuthenticatedEpersonErrorAction(error))
          )
        );
      })
    )
