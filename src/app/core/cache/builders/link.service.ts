...
export class LinkService {
  ...
  >(
  ...
  ): Observable<RemoteData<U | PaginatedList<U>>> {
    ...
    if (hasValue(matchingLinkDef)) {
      ...
      if (hasNoValue(provider)) {
        throw new Error(
          `The @link() for ${String(linkToFollow.name)} on ${model.constructor.name} models uses the resource type ${matchingLinkDef.resourceType.value.toUpperCase()}, but there is no service with an @dataService(${matchingLinkDef.resourceType.value.toUpperCase()}) annotation in order to retrieve it`,
        );
      }
      ...
      if (hasValue(link)) {
        ...
        try {
          ...
          console.error(
            `Something went wrong when using @dataService(${matchingLinkDef.resourceType.value}) ${hasValue(service) ? '' : '(undefined) '}to resolve link ${String(linkToFollow.name)} at ${href}`,
          );
          throw e;
        }
      }
      ...
      throw new Error(
        `followLink('${String(linkToFollow.name)}') was used as a required link for a ${model.constructor.name}, but there is no property on ${model.constructor.name} models with an @link() for ${String(linkToFollow.name)}`,
      );
    }
