...
describe('SubgroupsListComponent', () => {
  ...
  let mockActiveGroup: Group = Object.assign(new Group(), {
    ...
    _links: {
      ...
      subgroups: {
        href: 'https://rest.api/server/api/eperson/groups/activegroupid/subgroups',
      },
      object: {
        href: 'https://rest.api/server/api/eperson/groups/activegroupid/object',
      },
      epersons: {
        href: 'https://rest.api/server/api/eperson/groups/activegroupid/epersons',
      },
    },
    ...
    groupsDataServiceStub = {
      ...
      findListByHref(
        ...
        return createSuccessfulRemoteDataObject$(
          buildPaginatedList<Group>(
            new PageInfo(),
            groupsDataServiceStub.getSubgroups(),
          ),
        );
      },
      ...
      searchNonMemberGroups(
        ...
        if (query === '') {
          return createSuccessfulRemoteDataObject$(
            buildPaginatedList(new PageInfo(), groupNonMembers),
          );
        }
        return createSuccessfulRemoteDataObject$(
  ...
  describe('current subgroup list', () => {
    ...
    it('should show a delete button next to each subgroup', () => {
      const subgroupsFound = fixture.debugElement.queryAll(
        By.css('#subgroupsOfGroup tbody tr'),
      );
      subgroupsFound.map((foundGroupRowElement: DebugElement) => {
        const addButton: DebugElement = foundGroupRowElement.query(
          By.css('td:last-child .fa-plus'),
        );
        const deleteButton: DebugElement = foundGroupRowElement.query(
  ...
  describe('search', () => {
    describe('when searching with empty query', () => {
      ...
      it('should display only non-member groups (i.e. groups that are not a subgroup)', () => {
        const groupIdsFound: DebugElement[] = fixture.debugElement.queryAll(
          By.css('#groupsSearch tbody tr td:first-child'),
        );
        expect(groupIdsFound.length).toEqual(1);
        groupNonMembers.map((group: Group) => {
          expect(
            groupIdsFound.find((foundEl: DebugElement) => {
              return foundEl.nativeElement.textContent.trim() === group.uuid;
            }),
          ).toBeTruthy();
        });
      ...
      describe('if first add button is pressed', () => {
        beforeEach(() => {
          const addButton: DebugElement = fixture.debugElement.query(
            By.css('#groupsSearch tbody .fa-plus'),
          );
