...
export class AccessStatusBadgeComponent {
  ...
  ngOnInit(): void {
    ...
    if (
      this.object.type.toString() !== ITEM.value ||
    ...
    if (item.accessStatus == null) {
      ...
      item.accessStatus =
        this.accessStatusDataService.findAccessStatusFor(item);
    }
    this.accessStatus$ = item.accessStatus.pipe(
      map((accessStatusRD) => {
        if (
          accessStatusRD.statusCode !== 401 &&
          hasValue(accessStatusRD.payload)
        ) {
      ...
      map((accessStatus: AccessStatusObject) =>
        hasValue(accessStatus.status) ? accessStatus.status : 'unknown',
      ),
      map(
        (status: string) =>
    ...
    this.subs.push(
      this.accessStatus$
        .pipe(
          map((accessStatusClass: string) =>
            accessStatusClass.replace(/\./g, '-'),
          ),
        )
