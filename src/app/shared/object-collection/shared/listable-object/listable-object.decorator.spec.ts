...
describe('ListableObject decorator function', () => {
  ...
  beforeEach(() => {
    ...
    listableObjectComponent(
      ...
      undefined,
      undefined,
    )(TestDefaultLowPriorityComponent);
    listableObjectComponent(
      typeLowPriority,
      DEFAULT_VIEW_MODE,
      Context.Search,
      'custom',
    )(TestLowPriorityComponent);
    listableObjectComponent(
      typeLowPriority2,
      DEFAULT_VIEW_MODE,
      undefined,
    ...
    listableObjectComponent(
      ...
      undefined,
      undefined,
    )(TestDefaultMidPriorityComponent);
    listableObjectComponent(
      typeMidPriority,
      ViewMode.DetailedListElement,
      undefined,
      'custom',
    )(TestMidPriorityComponent);
    listableObjectComponent(
      typeHighPriority,
      ViewMode.DetailedListElement,
      Context.Search,
      undefined,
    )(TestHighPriorityComponent);
  ...
  describe('priorities', () => {
    ...
    describe('If multiple components exist, each containing a different default value for one of the requested parameters', () => {
      it('the component with the latest default value in the list should be returned', () => {
        let component = getListableObjectComponent(
          [typeMidPriority, typeLowPriority],
          ViewMode.DetailedListElement,
          Context.Search,
          'custom',
        );
        ...
        component = getListableObjectComponent(
          [typeLowPriority, typeMidPriority, typeHighPriority],
          ViewMode.DetailedListElement,
          Context.Search,
          'custom',
        );
    ...
    describe('If two components exist for two different types, both configured for the same view-mode, but one for a specific context and/or theme', () => {
      it('requesting a component for that specific context and/or theme while providing both types should return the most relevant one', () => {
        const component = getListableObjectComponent(
          [typeLowPriority2, typeLowPriority],
          ViewMode.DetailedListElement,
          Context.Search,
          'custom',
        );
