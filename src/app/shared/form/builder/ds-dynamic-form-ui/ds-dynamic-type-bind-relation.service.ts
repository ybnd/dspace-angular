...
export class DsDynamicTypeBindRelationService {
  constructor(
    ...
    private dynamicMatchers: DynamicFormControlMatcher[],
   ...
   * @private
   */
  public getTypeBindValue(
    bindModelValue: string | FormFieldMetadataValueObject,
  ): string {
    let value;
    if (hasNoValue(bindModelValue) || typeof bindModelValue === 'string') {
      value = bindModelValue;
    } else if (
      bindModelValue instanceof FormFieldMetadataValueObject &&
   ...
   * @param model
   */
  public getRelatedFormModel(
    model: DynamicFormControlModel,
  ): DynamicFormControlModel[] {
    const models: DynamicFormControlModel[] = [];

    (model as any).typeBindRelations.forEach((relGroup) =>
      relGroup.when.forEach((rel) => {
        if (model.id === rel.id) {
          throw new Error(`FormControl ${model.id} cannot depend on itself`);
        }

        const bindModel: DynamicFormControlModel =
          this.formBuilderService.getTypeBindModel();

        if (
          model &&
          !models.some((modelElement) => modelElement === bindModel)
        ) {
  ...
  public matchesCondition(
    ...
    return relation.when.reduce(
      (
        hasAlreadyMatched: boolean,
        condition: DynamicFormControlCondition,
        index: number,
      ) => {
        ...
        if (bindModel.type === DYNAMIC_FORM_CONTROL_TYPE_RELATION_GROUP) {
          bindModelValue = bindModel.value.map(
            (entry) => entry[bindModel.mandatoryField],
          );
        }
        // Support multiple bind models
        if (Array.isArray(bindModelValue)) {
          values = [
            ...bindModelValue.map((entry) => this.getTypeBindValue(entry)),
          ];
        } else {
  ...
  subscribeRelations(
    ...
    Object.values(relatedModels).forEach((relatedModel: any) => {
      if (hasValue(relatedModel)) {
        const initValue =
          hasNoValue(relatedModel.value) ||
          typeof relatedModel.value === 'string'
            ? relatedModel.value
            : Array.isArray(relatedModel.value)
              ? relatedModel.value
              : relatedModel.value.value;

        const updateSubject =
          relatedModel.type === 'CHECKBOX_GROUP'
            ? relatedModel.valueUpdates
            : relatedModel.valueChanges;
        const valueChanges = updateSubject.pipe(startWith(initValue));
          ...
          valueChanges.subscribe(() => {
            ...
            if (hasValue(this.dynamicMatchers)) {
              this.dynamicMatchers.forEach((matcher) => {
                // Find the relation
                const relation =
                  this.dynamicFormRelationService.findRelationByMatcher(
                    (model as any).typeBindRelations,
                    matcher,
                  );
