...
describe('RssComponent', () => {
  ...
  it('should formulate the correct url given params in url', () => {
    ...
    expect(route).toBe(
      '/opensearch/search?format=atom&scope=2cfcf65e-0a51-4bcb-8592-b8db7b064790&sort=dc.title&sort_direction=DESC&query=test'
    );
  });
  ...
  it('should skip uuid if its null', () => {
    ...
    expect(route).toBe(
      '/opensearch/search?format=atom&sort=dc.title&sort_direction=DESC&query=test'
    );
  });
  ...
  it('should default to query * if none provided', () => {
    ...
    expect(route).toBe(
      '/opensearch/search?format=atom&sort=dc.title&sort_direction=DESC&query=*'
    );
  });
});
