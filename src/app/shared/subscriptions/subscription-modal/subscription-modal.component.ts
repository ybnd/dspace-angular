...
export class SubscriptionModalComponent implements OnInit {
   ...
   */
  @Output() updateSubscription: EventEmitter<Subscription> =
  ...
  ngOnInit(): void {
    ...
    this.subscriptionForm.valueChanges.subscribe((newValue) => {
      ...
      for (const f of this.frequencyDefaultValues) {
        anyFrequencySelected =
          anyFrequencySelected || newValue.content.frequencies[f];
      }
  ...
  initFormByAllSubscriptions(): void {
    ...
    for (const t of this.subscriptionDefaultTypes) {
      ...
      for (const f of this.frequencyDefaultValues) {
        (formGroup.controls.frequencies as UntypedFormGroup).addControl(
          f,
  ...
  initFormByGivenSubscription(): void {
    ...
    formGroup.addControl(
      'subscriptionId',
      this.formBuilder.control(this.subscription.id),
    );
    formGroup.addControl('frequencies', this.formBuilder.group({}));
    (formGroup.get('frequencies') as UntypedFormGroup).addValidators(
      Validators.required,
    );
    for (const f of this.frequencyDefaultValues) {
      const value =
        findIndex(this.subscription.subscriptionParameterList, ['value', f]) !==
        -1;
      (formGroup.controls.frequencies as UntypedFormGroup).addControl(
        f,
  ...
  initFormDataBySubscriptions(): void {
    ...
    this.subscriptionService
      .getSubscriptionsByPersonDSO(this.ePersonId, this.dso?.uuid)
        ...
        next: (res: PaginatedList<Subscription>) => {
          if (res.pageInfo.totalElements > 0) {
            ...
            for (const subscription of res.page) {
              ...
              const subscriptionGroup: UntypedFormGroup =
                this.subscriptionForm.get(type) as UntypedFormGroup;
              if (isNotEmpty(subscriptionGroup)) {
                subscriptionGroup.controls.subscriptionId.setValue(
                  subscription.id,
                );
                for (const parameter of subscription.subscriptionParameterList.filter(
                  (p) => p.name === 'frequency',
                )) {
                  (
                    subscriptionGroup.controls.frequencies as UntypedFormGroup
                  ).controls[parameter.value]?.setValue(true);
                }
              }
            }
          }
  ...
  submit() {
    this.submitted = true;
    const subscriptionTypes: string[] = Object.keys(
    ...
    subscriptionTypes.forEach((subscriptionType: string) => {
      const subscriptionGroup: UntypedFormGroup = this.subscriptionForm
        .controls[subscriptionType] as UntypedFormGroup;
    ...
    if (isNotEmpty(subscriptionsToBeCreated)) {
      toBeProcessed.push(
        from(subscriptionsToBeCreated).pipe(
          ...
          tap((res: RemoteData<Subscription>) => {
            if (res.hasSucceeded) {
              const msg = this.translate.instant(
                'subscriptions.modal.create.success',
              ...
              this.notificationsService.error(
                null,
                this.translate.instant('subscriptions.modal.create.error'),
              );
            }
          }),
        ),
    ...
    if (isNotEmpty(subscriptionsToBeUpdated)) {
      toBeProcessed.push(
        from(subscriptionsToBeUpdated).pipe(
          mergeMap((subscriptionBody) => {
            return this.subscriptionService
              .updateSubscription(
                subscriptionBody,
          ...
          tap((res: RemoteData<Subscription>) => {
            if (res.hasSucceeded) {
              const msg = this.translate.instant(
                'subscriptions.modal.update.success',
                { type: res.payload.subscriptionType },
              );
              this.notificationsService.success(null, msg);
              if (isNotEmpty(this.subscription)) {
              ...
              this.notificationsService.error(
                null,
                this.translate.instant('subscriptions.modal.update.error'),
              );
            }
          }),
        ),
