...
describe('DSOEditMenuResolver', () => {
  ...
  beforeEach(waitForAsync(() => {
    ...
    dsoVersioningModalService = jasmine.createSpyObj(
    ...
    TestBed.configureTestingModule({
      ...
      providers: [
        ...
        {
          provide: DsoVersioningModalService,
          useValue: dsoVersioningModalService,
        },
        {
  ...
  describe('resolve', () => {
    it('should create all menus when a dso is found based on the route id param', (done) => {
      spyOn(resolver, 'getDsoMenus').and.returnValue([
        observableOf(dummySections1),
        observableOf(dummySections2),
      ]);
      resolver.resolve(route as any, null).subscribe((resolved) => {
        expect(resolved).toEqual({
          ...route.data.menu,
          [MenuID.DSO_EDIT]: [
            ...dummySections1.map((menu) =>
              Object.assign(menu, { id: menu.id + '-test-item-uuid' }),
            ),
            ...dummySections2.map((menu) =>
              Object.assign(menu, { id: menu.id + '-test-item-uuid' }),
            ),
          ],
        });
        expect(dSpaceObjectDataService.findById).toHaveBeenCalledWith(
          'test-item-uuid',
          true,
          false,
        );
    ...
    it('should create all menus when a dso is found based on the route scope query param when no id param is present', (done) => {
      ...
      resolver.resolve(routeWithScope as any, null).subscribe((resolved) => {
        expect(resolved).toEqual({
          ...
          [MenuID.DSO_EDIT]: [
            ...dummySections1.map((menu) =>
              Object.assign(menu, { id: menu.id + '-test-scope-uuid' }),
            ),
            ...dummySections2.map((menu) =>
              Object.assign(menu, { id: menu.id + '-test-scope-uuid' }),
            ),
    ...
    it('should return the statistics menu when no dso is found', (done) => {
      (dSpaceObjectDataService.findById as jasmine.Spy).and.returnValue(
        createFailedRemoteDataObject$(),
      );
  ...
  describe('getDsoMenus', () => {
    describe('for Communities', () => {
      beforeEach(() => {
        ...
        dSpaceObjectDataService.findById.and.returnValue(
          createSuccessfulRemoteDataObject$(testCommunity),
        );
      ...
      it('should not return Item-specific entries', (done) => {
        ...
        combineLatest(result)
          ...
          .subscribe((menu) => {
            ...
            const versionEntry = menu.find(
              (entry) => entry.id === 'version-dso',
            );
      ...
      it('should return Community/Collection-specific entries', (done) => {
        ...
        combineLatest(result)
          .pipe(map(flatten))
          .subscribe((menu) => {
            const subscribeEntry = menu.find(
              (entry) => entry.id === 'subscribe',
            );
    ...
    describe('for Items', () => {
      beforeEach(() => {
        testObject = testItem;
        dSpaceObjectDataService.findById.and.returnValue(
          createSuccessfulRemoteDataObject$(testItem),
        );
        route = dsoRoute(testItem);
      });
