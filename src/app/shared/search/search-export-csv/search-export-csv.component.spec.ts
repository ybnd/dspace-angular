...
describe('SearchExportCsvComponent', () => {
  ...
  const script = Object.assign(new Script(), {
    id: 'metadata-export-search',
    name: 'metadata-export-search',
  });
  const process = Object.assign(new Process(), {
    processId: 5,
    scriptName: 'metadata-export-search',
  });
  ...
  const searchConfig = new PaginatedSearchOptions({
    ...
    query: 'test-query',
    filters: [
      new SearchFilter('f.filter1', [
        'filter1value1,equals',
        'filter1value2,equals',
      ]),
  ...
  describe('init', () => {
    ...
    describe('when the metadata-export-search script is not present', () => {
      beforeEach(waitForAsync(() => {
        initBeforeEachAsync();
        (scriptDataService.findById as jasmine.Spy).and.returnValue(
  ...
  describe('export', () => {
    ...
    it('should show a success message when the script was invoked successfully and redirect to the corresponding process page', () => {
      ...
      expect(router.navigateByUrl).toHaveBeenCalledWith(
        getProcessDetailRoute(process.processId)
      );
    });
