  ...
  implements OnInit
{
  ...
  ngOnInit() {
    super.ngOnInit();
    this.vocabularyExists$ = this.vocabularyService
      .searchTopEntries(this.getVocabularyEntry(), new PageInfo(), true, false)
      .pipe(
  ...
  showVocabularyTree() {
    ...
    modalRef.result
      .then((detail: VocabularyEntryDetail) => {
   ...
   */
  getVocabularyEntry() {
    const foundVocabularyConfig = environment.vocabularies.filter(
      (v) => v.filter === this.filterConfig.name,
    );
    if (
      foundVocabularyConfig.length > 0 &&
      foundVocabularyConfig[0].enabled === true
    ) {
